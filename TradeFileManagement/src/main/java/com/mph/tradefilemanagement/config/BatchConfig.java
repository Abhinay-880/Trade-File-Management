...
public class BatchConfig {
	...
	public BatchConfig(FileLoadRepository fileLoadRepository) {
	...
	private LineMapper<FileLoad> lineMapper() {
		...
		tokenizer.setNames("filename", "status", "recordcount", "errors");
	...
	public Step step1(JobRepository jobRepository, PlatformTransactionManager transactionManager,
			FlatFileItemReader<FileLoad> itemReader) {
		return new StepBuilder("step1", jobRepository).<FileLoad, FileLoad>chunk(10, transactionManager)
				.reader(itemReader).processor(processor()).writer(writer()).build();
	}

	@Bean
	public Job runJob(JobRepository jobRepository, Step step1) {
		return new JobBuilder("runJob", jobRepository).start(step1).build();
	}
